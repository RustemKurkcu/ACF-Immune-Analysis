# DEseq2 with replicates combined
...
# filter UMI table, if a gene is expressed in < 50% samples, will drop
...
ddscollapsed.e <- collapseReplicates(
  dds.e,
  groupby = dds.e$Sample
)

ddscollapsed.e<- DESeq(ddscollapsed.e)
res.e <- results(ddscollapsed.e)
...
write.csv(res.e,"DESeq2_output.epi.collapsed.ACF_vs_Norm.csv")
...
ddscollapsed.s <- collapseReplicates(
  dds.s,
  groupby = dds.s$Sample
)

ddscollapsed.s<- DESeq(ddscollapsed.s)
res.s <- results(ddscollapsed.s)
...
write.csv(res.s,"DESeq2_output.stromal.collapsed.ACF_vs_Norm.csv")
